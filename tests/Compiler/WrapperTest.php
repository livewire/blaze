...
test('wraps component templates into function definitions', function () {
    ...
    expect($wrapped)->toEqualCollapsingWhitespace(join('', [
        ...
        '$__env = $__blaze->env; ',
        'if (!isset($attributes)) { ',
        '$attributes = \Livewire\Blaze\Runtime\BlazeAttributeBag::make($__data, [], []); ',
        '} elseif ($attributes instanceof \Illuminate\View\ComponentAttributeBag && ! $attributes instanceof \Livewire\Blaze\Runtime\BlazeAttributeBag) { ',
...
test('wrap includes view-render trigger that calls the function', function () {
    ...
    expect($wrapped)
        ...
        ->toContain('if (!isset($attributes))')
        ->toContain('<div></div>')
        ->not->toContain("_{$hash}(\$__blaze, \$__data, [], [], [], null")
        ->not->toContain('$__capture')
